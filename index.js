...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Jadz🌴\n' // GANTI NAMA LU
            + 'ORG:OWNER BOTZ;\n'
            + 'TEL;type=CELL;type=VOICE;waid=558281083059:+55 82 8108 3059\n' // GANTI NOMOR LU
            + 'END:VCARD'
...
console.log(banner.string)
   ...
   qrcode.generate(qr, { small: true })
	console.log(color('[','white'), color('!','red'), color(']','white'), color(' É o JADZ Crlh🙄'))
})
	...
	baby.on('connecting', () => {
		start('2', 'Prismo connecting...')
	})
	baby.on('open', () => {
		success('2', 'Prismo Connected')
	})
...
*/
				...
				var nin = gh.split("&")[0];
				var ja = gh.split("&")[1];
				if (args.length < 1) return reply(`「❗」Exemplo : ${prefix}ninjalogo Jadz & Gans`)
				reply(nad.wait())
		...
		case 'halloweentext':
		    ...
		    break
				...
				var porn = gh.split("&")[0];
				var hub = gh.split("&")[1];
				if (args.length < 1) return reply(`「❗」Exemplo : ${prefix}pornhub Jadz & Hub`)
				reply(nad.wait())
		...
		case 'textlight':
		    ...
		    break
				...
				if (isLimit(sender)) return reply(nad.limitend(pusname))
					...
					var gli = gh.split("&")[0];
					var tch = gh.split("&")[1];
					if (args.length < 1) return reply(`「❗」Exemplo : ${prefix}glitchtext Jadz & Gans`)
					reply(nad.wait())
				...
				if (!isRegistered) return reply(nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
				if (!isQuotedSticker) return reply('Responda ou marque o adesivo que você deseja fazer uma foto🙄')
					reply(nad.wait())
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						if (err) return reply(nad.stikga())
						buffer = fs.readFileSync(ran)
						baby.sendMessage(from, buffer, image, {quoted: mek, caption: 'Aqui esta [(^.^)]'})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
					break
                ...
                var gh = body.slice(12)
					...
					baby.sendMessage(from, buffer, image, {caption: 'Aqui esta :D', quoted: mek})
					await limitAdd(sender)
					break
                case 'meme':
                if (isBanned) return reply(nad.baned())
...
// PREMIUM
				case 'premiumlist':
					baby.updatePresence(from, Presence.composing) 
                    if (!isRegistered) return reply(nad.noregis())
					teks = `╭─「 *LISTA DE USUÁRIOS PREMIUM* 」\n`
					no = 0
...
case 'neontext':
				...
				if (!isRegistered) return reply(nad.noregis())
				if (!isPrem) return reply(nad.premium())
					if (args.length < 1) return reply(`「❗」Exemplo : ${prefix}neontext Ramlan`)
					neon = body.slice(10)
					reply('「❗」Espere um pouco!')
...
*/		 
				case 'welcome':
                  if (isBanned) return reply(nad.baned())				
					...
					if (args.length < 1) return reply('Ta ativo🙄')
					if (Number(args[0]) === 1) {
...
*/				
			...
			break
				...
				if (isLimit(sender)) return reply(nad.limitend(pusname))
					...
					pok = await getBuffer(nimek)
					baby.sendMessage(from, pok, image, { quoted: mek, caption: `*⟪Tinha muitos, mas peguei esse :v ⟫*`})
					await limitAdd(sender)
					break 
                case 'beritahoax':
                  if (isBanned) return reply(nad.baned())
...
if (!isRegistered) return reply(nad.noregis())
if (isLimit(sender)) return reply(nad.limitend(pusname))
if (args.length < 1) return reply(`「❗」Exemplo : ${prefix}hartatahta Ramlan`)
har = body.slice(12)
...
case 'cloudtext':
				...
				if (!isRegistered) return reply(nad.noregis())
				if (!isPrem) return reply(nad.premium())
if (args.length < 1) return reply(`「❗」Exemplo : ${prefix}cloudtext Ramlan`)
cloud = body.slice(11)
...
*/
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							baby.sendMessage(_.jid, buff, image, {caption: `*「 PRISMO BROADCAST 」*\n\n${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `*「 PRISMO TRANSMISSAO 」*\n\n${body.slice(4)}`)
						}
				...
				case 'bcgc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await baby.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							baby.sendMessage(_.jid, buff, image, {caption: `*「 BC GRUPO 」*\n\n➸ Dari Grup : ${groupName}\n➸ Pengirim : wa.me/${(sender.split('@')[0])}\n➸ Pesan : ${body.slice(6)}`})
						}
						reply('*「 SUCESSO EM TRANSMITIR 」*')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `*「 BC GRUPO 」*\n\n➸ Dari Grup : ${groupName}\n➸ Pengirim : wa.me/${(sender.split('@')[0])}\n➸ Pesan : ${body.slice(6)}`)
						}
						reply('*「 SUCESSO NA TRANSMISSAO 」*')
					}
					break
					case 'setreply':
					if (!isOwner) return reply(nad.ownerb())
